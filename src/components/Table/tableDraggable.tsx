...
const TableForm: React.FC<{ initialData?: Table; onSave?: () => void }> = ({ initialData, onSave }) => {
  ...
  const handleDragStop = (index: number, droppedIndex: number) => {
    if (draggedGuestIndex !== null && draggedGuestIndex !== droppedIndex) {
      const updatedSeatAssignments = [...seatAssignments];
      const draggedGuest = updatedSeatAssignments.splice(draggedGuestIndex, 1)[0];
      updatedSeatAssignments.splice(droppedIndex, 0, draggedGuest);
  ...
  const renderSeats = () => {
    ...
    const topRow = Math.floor(capacity / 2);
    ...
    for (let i = 0; i < capacity; i++) {
      ...
      const topPercentage = isTopRow ? '20%' : '70%';
      const leftPercentage = `${(100 / (rowLength + 1)) * (rowIndex + 1)}%`;
  ...
  const renderDraggableGuests = () => {
    return guests.map((guest, index) => (
      <Draggable
        ...
        onStop={(e, data) => handleDragStop(index, Math.floor(data.x / 100))}
      >
        <div className="w-24 h-12 bg-gray-200 text-gray-800 rounded flex items-center justify-center m-2 cursor-pointer">
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-lg shadow-lg p-6 mb-6">
      ...
      <div className="mt-6">
        <h3 className="text-2xl mb-4">Guest List</h3>
        <div className="flex flex-wrap">{renderDraggableGuests()}</div>
      </div>
    </form>
  );
};
